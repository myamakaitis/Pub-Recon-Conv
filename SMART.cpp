...
)
{
    ...
    ElementWiseExp2_InPlace_Double(M*N, IMG, 1.0);
...
)
{
    ...
    int ImgSize = M*N;
    float ImgSizeinv2 = (1 / static_cast<float>(ImgSize)); //*(1 / static_cast<float>(ImgSize));
    float ImgSizeinvlog = std::log2f(1 / static_cast<float>(ImgSize));
    ...
    ElementWiseLog2_InPlace_Float(M*N, IMG, 1.0);

    for (int iter = 0; iter < ITERS; iter++){

        ProjForward_Float(L, M, N, FFT2d_Forward_Batch, Vol_FT, PSF_FT, FFT2d_Backward);

        ElementWiseLog2_InPlace_Float(M*N, FP, ImgSizeinvlog);
        ...
        ElementWiseExp2_InPlace_Float(M*N*L, Vol_Temp, ImgSizeinv2);
    ...
    ElementWiseExp2_InPlace_Float(M*N, IMG, 1.0);
